...
abstract class RWMB_Field
{
	...
	static function show( $field, $saved )
	{
		...
		$data_required_field = '';
		if (!empty($field['required-field']) && is_array($field['required-field']) && (count($field['required-field']) == 3)) {
			$data_required_field = 'data-required-ref="' . esc_attr($field['required-field'][0]) . '"';
			$data_required_field .= ' data-required-operator="' . esc_attr($field['required-field'][1]) . '"';
			if (!is_array($field['required-field'][2])) {
				$data_required_field .= ' data-required-value="' . esc_attr($field['required-field'][2]) . '"';
			}
			else {
				$data_required_field .= ' data-required-value="' . esc_attr(join(',',$field['required-field'][2])) . '"';
			}

		}

		$outer_html = sprintf(
			$field['before'] . '<div %s id="%s" class="%s">%s</div>' . $field['after'],
			$data_required_field,
			trim($div_id),
			trim( $classes ),
			$html
	...
	{
		$text = apply_filters( 'rwmb_add_clone_button_text', esc_html__( '+ Add more', 'pl-emallshop-extensions' ), $field );
