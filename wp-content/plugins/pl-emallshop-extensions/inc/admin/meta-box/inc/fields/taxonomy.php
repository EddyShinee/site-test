...
class RWMB_Taxonomy_Field extends RWMB_Object_Choice_Field
{
	...
	static function normalize( $field )
	{
		...
		{
			$field['placeholder'] = esc_html__( 'Select a term', 'pl-emallshop-extensions' );
			if ( is_string( $field['taxonomy'] ) && taxonomy_exists( $field['taxonomy'] ) )
			{
				$taxonomy_object      = get_taxonomy( $field['taxonomy'] );
				$field['placeholder'] = sprintf( esc_html__( 'Select a %s', 'pl-emallshop-extensions' ), $taxonomy_object->labels->singular_name );
			}
		}
