...
class RWMB_Autocomplete_Field extends RWMB_Multiple_Values_Field
{
	...
	static function admin_enqueue_scripts()
	{
		...
		wp_localize_script( 'rwmb-autocomplete', 'RWMB_Autocomplete', array( 'delete' => esc_html__( 'Delete', 'pl-emallshop-extensions' ) ) );
	}
			...
			foreach ( $field['options'] as $value => $label )
			{
				if ( in_array( $value, $meta ) )
				{
					$html .= sprintf(
						$tpl,
						$label,
						esc_html__( 'Delete', 'pl-emallshop-extensions' ),
						$field['field_name'],
						$value
					);
				}
