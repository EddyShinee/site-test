...
class Emallshop_Dashboard {
	...
	function __construct() {
        ...
        $this->doc_link 			= 'http://docs.presslayouts.com/emallshop';
        $this->demo_preview_link 	= 'https://demo.presslayouts.com/emallshop';
        $this->changelog_link 		= 'https://demo.presslayouts.com/emallshop'; 
	...
	public function is_emallshop_server_available(){
		global $wp_version;
		...
		if ( ! isset( $installed_plugins[ $item['file_path'] ] ) ) {
			...
			$actions = sprintf( '<a href="%1$s" title="%2$s">Activate</a>',
				esc_url( add_query_arg( array(
					...
					admin_url( 'admin.php?page=emallshop-theme' ) ) ),
				$item['sanitized_plugin'] );
			...
			$actions = sprintf( '<a href="%1$s" title="%2$s">Update</a>',
				wp_nonce_url( add_query_arg( array(
					...
					'tgmpa-nonce' ),
				$item['sanitized_plugin'] );
		} elseif ( emallshop_check_plugin_active( $item['file_path'] ) ) {
	...
	public function emallshop_let_to_num( $size ) {
		$l   = substr( $size, -1 );
		$ret = substr( $size, 0, -1 );
		switch ( strtoupper( $l ) ) {
			case 'P':
